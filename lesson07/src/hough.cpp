...
cv::Mat buildHough(cv::Mat sobel) {// единственный аргумент - это результат свертки Собелем изначальной картинки
    ...
    cv::Mat accumulator(max_r, max_theta - 1, CV_32FC1); // TODO подумайте какого разрешения она должна быть и поправьте ее размер
    ...
    for (int y0 = 0; y0 < height; ++y0) {
        for (int x0 = 0; x0 < width; ++x0) {
            ...
            for (int theta0 = 1; theta0 < max_theta - 1; ++theta0) {
